...
import { getFilteredCoordinates, intercectionHelper, unionHelper } from '../utils';
...
export function useSelectedCoordinates() {
  ...
  useEffect(() => {
    ...
    const geometryList = polygoneMode === 'union' ? unionHelper(filteredCoords) : intercectionHelper(filteredCoords);
